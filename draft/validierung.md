...
Wir könnten natürlich einen Konstruktor für `User`
schreiben, der solche Fehler abfängt.  Wir benutzen für das Ergebnis
`Either a b = Left a | Right b`:

KAAN: "hält uns nichts davon ab" zweimal hintereinander.
...
```haskell
combineValidations :: Validation a -> Validation b -> Validation c
...
KAAN: Ich verstehe hier nicht, wieso wir `applyOkFunctionToValidation`
brauchen. Also, warum machen wir nicht einfach mit

```haskell
combineValidations (OK x) (Fail y) = ...
```
...
Verfügung.  Wir können aber Funktionen mit
Argumenten füttern und schauen, was rauskommt (oder im Fall von
Haskell so lange probieren, bis der Compiler sein Okay gibt).  Uns
bleibt an dieser Stelle also nicht viel anderes übrig, als einfach so
zu tun, als stünde links ein `Ok` dessen Wert eine Funktion ist, die
weiß, was mit dem Wert rechts zu tun ist.  Wenn wir das einmal kurz
schlucken, können wir uns dafür eine kleine Hilfsfunktion schreiben.

KAAN: Zweimal "Nicht viel anderes" hintereinander
...
KAAN: "indem das hin das linke `OK` gewickelte" -- links ist doch eine
normale Funktion `a -> b`, ohne Wicklung? AHH Du sprichst hier dann
von `combineValidations`?!
...
1. jede Validierungen für sich durchführen können -> wir schreiben
   Funktionen, die `Validation`s als Ergebnis haben
