...
zu Problemen, spätestens beim Testen von Software. Daher soll in diesem Artikel anhand von 
...
Alle Code-Beispiele in diesem Artikel sind mit [Scala](https://de.wikipedia.org/wiki/Scala_(Programmiersprache)) implementiert. Da dieser Artikel praxis-
orientiert ist, werden die Beispiele unter Zuhilfenahme der Bibliothek [Cats](https://typelevel.org/cats/) implementiert. Zwar wird die freie Monade nicht theoretisch erklärt, dennoch sollte das 
...
wir eine case-Klasse, die eine Adresse repräsentiert und ein Objekt, dass Funktion für die Persistierung selbiger anbietet: 
...
* *Seiteneffekt werden sofort ausgeführt*: Ein Grundgedanke der funktionalen Programmierung ist die Erstellung von referentiell transparenten Programmen, also ohne Seiteneffekte. Diese Seiteneffekt sollen möglichst erst "am Ende des Universum" ausgeführt werden. Im Beispiel würden Effekte verstreut an vielen Stellen im Programm ausgeführt werden. Insbesondere würde die Funktion removeBielefeld bei Mehrfachausführung unterschiedliche Ergebnisse liefern.
* *Die Schnittstelle ist zu speziell:* Die Trennung von Beschreibung und Ausführung ist ein wichtiges Konzept, das zu modularem, exzellent wartbarem und testbarem Code führt. Bei der Beschreibung unserer Adressbuch-Operationen sollten wir uns nicht mit Futures, Eithers und Throwables abmühen müssen. Mehr noch, setzen wir damit ein bestimmtes Datenbank voraus (in diesem Fall ein asynchrones, dass Exceptions verwendet), das damit zu stark in der Business-Logik unserer Anwendung verzahnt und somit nur schwer austauschbar wäre.
...
Denkbar wäre jedoch einen einfachen Interpreter zu implementieren, der die ausgeführten Anweisungen lediglich traced. Allerdings muss hierzu in einigen Fällen trotzdem das Verhalten implementiert werden, zum Beispiel wenn bestimmte Operationen nur bedingt durch gebundene Daten ausgeführt werden.
...
In diesem Blog-Artikel haben wir entlang eines einfachen Beispiels eine kleine Sprache in Form der freien Monade implementiert. Diese Sprache hat es uns ermöglicht, kombinierbarer Beschreibungen ohne Wissen über die spätere Ausführung zu formulieren. Erst durch Intepreter (oder Compiler) werden die Beschreibungen ausgeführt und Details über die Ausführung bekannt. 
